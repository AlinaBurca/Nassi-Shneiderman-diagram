...
void TransfInArb(arbore &A, string &cod, int st, int dr)
{
	if(st<=dr)
	{
    	...
    	{
        	if((cod[st]=='-' && cod[st-1]=='-')||(cod[st]==39 && cod[st-1]==39))/// punem spatiu it=ntre minusuri si ghilimele
            	instr+=' ';
        	if(cod[st]!=' ')
            	instr+=cod[st];
        	++st;
...
void deseneaza10(arbore &A, int x1, int y1, int x2, int y2, int lineH, int spatiuSt, int maxlat)/// switch si case
{
	...
	while(p!=NULL)
	{
    	...
    	xv= xv+ maxlat; /// schimbam coordonatele triunghiului
    	yv=yv+lineH;
...
void deseneaza(nodL *L, int lineH, int spatiuSt, int x1, int y1, int x2, int y2)
{
	...
	{
    	///aici nu avem conditie ca fiu al lui do
    	xSt+=spatiuSt;
	}
	///setam ySt si yDr
    	...
    	if(A->tip==3)
        	if(L->urm!=NULL)
            	if(L->urm->fiu->tip==4)
            	{
                	i=L->urm->fiu->L.prim;
                	nrlIf=nrLiniiNecIfSauElse(i);
                	newLineH=(y2-y1-lineH)/nrlIf; /// inaltimea unei linii difera in functie de lineH
                	xSt=xDr;
                	xDr=x2;
                	ySt=yDr=y1+ lineH;
                	while(i!=NULL)
...
void scrollCircle(int fctCurenta, int nrFunctii)
{
	...
	for(int i=0; i<=nrFunctii; i++)
	{
    	if(i==fctCurenta)
    	{
        	...
        	bar(x-raza , y-raza, x+raza, y+raza);
...
void deseneazaTot(arbore A[], int nrFunctii)
{
	...
	while(key!='q')
	{
    	...
    	setfillstyle(SOLID_FILL, BLACK); /// culori diagrama
    	bar(0, 0, scrW, scrH);
    	setfillstyle(SOLID_FILL, WHITE);
